...
display_usage() {
    ...
    echo -e "${BOLD_YELLOW}    $0 -ml path/to/subdomains.txt${NC}"
...
check_tools() {
    tools=("gau" "waymore" "waybackurls" "katana" "urlfinder" "reflection" "anew" "qsreplace" "match" "pm" "subprober")
...
if [[ "$1" == "-l" ]]; then
    ...
    urlfinder -all -d "$domain_Without_Protocol" -o $base_dir/urlfinder.txt || handle_error "urlfinder execution problem"
    if [[ $? -eq 0 ]]; then
    ...
    katana -u "$domain_Without_Protocol" -rl 170 -timeout 5 -retry 2 -aff -d 4 -duc -ps -pss waybackarchive,commoncrawl,alienvault -o $base_dir/katana.txt || handle_error "katana execution problem"
    if [[ $? -eq 0 ]]; then
    ...
    waybackurls "$domain_Without_Protocol" | tee $base_dir/waybackurls.txt || handle_error "waybackurls execution problem"
    if [[ $? -eq 0 ]]; then
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo -e "${BOLD_BLUE} Moving old files to trash ${NC}${RED}(urlfinder.txt, waymore.txt, gau.txt, katana.txt, waybackurls.txt).${NC}"
        mv $base_dir/urlfinder.txt $base_dir/waymore.txt $base_dir/katana.txt $base_dir/gau.txt $base_dir/waybackurls.txt $trash_dir
        echo ""
    else
        echo -e "${RED}❌ All urls merging failed.${NC}"
    fi
    # sensitive urls
    cat $base_dir/all_unique_urls.txt | grep -aE '\.xls|\.xml|\.xlsx|\.json|\.pdf|\.sql|\.doc|\.docx|\.pptx|\.txt|\.zip|\.tar\.gz|\.tgz|\.bak|\.7z|\.rar|\.log|\.cache|\.secret|\.db|\.backup|\.yml|\.gz|\.config|\.csv|\.yaml|\.md|\.md5|\.exe|\.dll|\.bin|\.ini|\.bat|\.sh|\.tar|\.deb|\.rpm|\.iso|\.img|\.apk|\.msi|\.dmg|\.tmp|\.crt|\.pem|\.key|\.pub|\.asc' | anew | tee $base_dir/sensitive_urls.txt
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo -e "${BOLD_BLUE} Moving old files to trash${NC}${RED}(urlfinder_uro.txt, waymore_uro.txt, gau_uro.txt, katana_uro.txt, waybackurls_uro.txt).${NC}"
        mv $base_dir/urlfinder_uro.txt $base_dir/waymore_uro.txt $base_dir/katana_uro.txt $base_dir/gau_uro.txt $base_dir/waybackurls_uro.txt $trash_dir
        echo ""
    else
    ...
    # remove web ports from urls
    match -f $base_dir/get_params2.txt -p "http" -r -v -o $base_dir/All_get_params1.txt || handle_error "match get params problem"
    if [[ $? -eq 0 ]]; then
        # count parameter urls after delete javascript urls
        All_get_params1=$(cat $base_dir/All_get_params1.txt | wc -l)
        echo -e "${BOLD_YELLOW}🔍 match based get parameters are: ${NC} ${RED}$All_get_params1${NC}"
        sleep 1
    else
        echo -e "${RED}❌ match get params failed.${NC}"
    fi

    # live params finding
    subprober -f $base_dir/All_get_params1.txt -sc -nc -mc 200,201,202,204,301,302,304,307,308,403,500,504,401,407 -c 100 -o $base_dir/All_get_params2.txt || handle_error "subprober problem"

    cat $base_dir/All_get_params2.txt | awk '{print $1}' | anew > $base_dir/All_get_params_final.txt || handle_error "Final get params merging problem"

    reflection -f $base_dir/All_get_params_final.txt -p "RXSS" -o $base_dir/all_reflected_params.txt || handle_error "reflection problem"
    if [[ $? -eq 0 ]]; then
    ...
    pm -f $base_dir/All_get_params_final.txt -p /opt/Pattern-Matching/patterns/xss.txt -c -i -r "BXSS" -o $base_dir/get_params_bxss.txt || handle_error "Pattern Matching bxss get params problem"
    echo -e "${BOLD_YELLOW}🔍 GF parameters found${NC}"

    pm -f $base_dir/All_get_params_final.txt -i -r "SQLI" -p /opt/Pattern-Matching/patterns/sqli.txt -o $base_dir/sqli_live_params.txt
    echo -e "${BOLD_YELLOW}🔍 SQLi parameters found${NC}"

    pm -f $base_dir/All_get_params_final.txt -c -i -r "OPENREDIRECT" -p /opt/Pattern-Matching/patterns/or.txt -o $base_dir/or_live_params.txt
    echo -e "${BOLD_YELLOW}🔍 Open redirect parameters found${NC}"

    pm -f $base_dir/All_get_params_final.txt -c -i -r "LFI" -p /opt/Pattern-Matching/patterns/lfi.txt -o $base_dir/lfi_live_params.txt
    echo -e "${BOLD_YELLOW}🔍 lfi parameters found${NC}"

    mv $base_dir/all_uro_unique_urls.txt $base_dir/get_params2.txt $base_dir/All_get_params1.txt $base_dir/All_get_params2.txt $trash_dir
    echo ""
    ...
    # Sensitive urls finding
    sensitive_url_path=$base_dir/sensitive_urls.txt
    sensitive_url_count=$(cat $base_dir/sensitive_urls.txt | wc -l)
    echo -e "${BOLD_YELLOW}🔍 ✅ All sensitive urls${NC}(${RED}$sensitive_url_count${NC}): ${BOLD_BLUE}$sensitive_url_path${NC}"


    # All live parameters printing
    final_get_post_live_params_path=$base_dir/All_get_params_final.txt
    final_get_post_live_params=$(cat $base_dir/All_get_params_final.txt | wc -l)
    echo -e "${BOLD_YELLOW}🔍✅ Final get live parameters are${NC}(${RED}$final_get_post_live_params${NC}): ${BOLD_BLUE}$final_get_post_live_params_path${NC}"
    sleep 1

    # GF live parameters
    GF_live_params_path=$base_dir/get_params_bxss.txt
    GF_live_params_count=$(cat $base_dir/get_params_bxss.txt | wc -l)
